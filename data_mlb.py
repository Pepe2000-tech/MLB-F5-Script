...
HEADERS = {"User-Agent": "MLB-Betting-Hub/3.2"}
...
def get_team_form(team_id,target_date):
    ...
    start=target.replace(month=3,day=20)
...
def get_hitter_stats(player_id,season,opposing_hand="R"):
    ...
    try:
        ...
        if splits:
            ...
            overall={
                "ops":_f(s.get("ops"),.720),"pa":pa,"ab":ab,"hits":hits,"hr":hr,
                "runs":runs,"rbi":rbi,"tb":tb,"so":so
            }
    except Exception:
        pass
    ...
    if not overall:
        return {
            "ops":.720,"pa":0,"hits":0,"hr":0,"runs":0,"rbi":0,"tb":0,"so":0,
    ...
    return {
        **overall,
        "ops":ops,
        "hit_rate":overall["hits"]/pa,
        "hr_rate":overall["hr"]/pa,
        "tb_rate":overall["tb"]/pa,
        "hrr_rate":(overall["hits"]+overall["runs"]+overall["rbi"])/pa,
        "k_rate":overall["so"]/pa,
        "used_split":bool(split and split["pa"]>=30),
