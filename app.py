...
HEADERS={"User-Agent":"MLB-Betting-Hub/7.6.6"}
...
def _write_draftea_lines_v766(rows):
    try:
...
def draftea_line_overrides_v766():
    if "v766_draftea_market_lines" not in st.session_state:
        st.session_state["v766_draftea_market_lines"]=_load_draftea_lines_v766()
    return st.session_state["v766_draftea_market_lines"]

def draftea_line_key_v766(game_pk,market_family,subject,bet_date=None):
    return f"{bet_date or ''}|{int(game_pk or 0)}|{str(market_family or '').lower()}|{str(subject or '').strip().lower()}"

def save_draftea_line_v766(item,line,bet_date=None):
    ...
    key=draftea_line_key_v766(item.get("game_pk"),fam,item.get("subject"),bet_date)
    rows=dict(draftea_line_overrides_v766())
    rows[key]={"line":float(line),"updated_at":datetime.now(CDMX_TZ).isoformat(timespec="seconds")}
    st.session_state["v766_draftea_market_lines"]=rows
    return _write_draftea_lines_v766(rows)

def delete_draftea_line_v766(item,bet_date=None):
    key=draftea_line_key_v766(item.get("game_pk"),item.get("market_family"),item.get("subject"),bet_date)
    rows=dict(draftea_line_overrides_v766())
    existed=key in rows
    rows.pop(key,None)
    st.session_state["v766_draftea_market_lines"]=rows
    _write_draftea_lines_v766(rows)
    return existed

def get_draftea_line_v766(item,bet_date=None):
    key=draftea_line_key_v766(item.get("game_pk"),item.get("market_family"),item.get("subject"),bet_date)
    row=draftea_line_overrides_v766().get(key)
...
def enrich_candidates_reference_odds(candidates,games_list):
    ...
    for (gpk,eid),meta in groups.items():
        ...
        for i in meta["idx"]:
            ...
            # V7.6.7: la línea REAL informada por el usuario en Draftea tiene prioridad sobre
...
def market_display_label_v766(item):
...
def _reprice_from_group_v766(group,line,side):
    base=next((z for z in group if z.get("sample_values") is not None),None)
    if base is None: return None
    y=v7_reprice_line(base,float(line),side)
    if y is None: return None
    y["label"]=market_display_label_v766(y)
    return y

def normalize_draftea_prop_candidates_v766(items,game_pk=None,bet_date=None):
    ...
    for (fam,subj),grp in groups.items():
        ...
        override=get_draftea_line_v766(probe,bet_date)
        if override is not None:
            sides=["over","under"] if fam=="pitcher_k" else ["over"]
            for side in sides:
                y=_reprice_from_group_v766(grp,override,side)
                if y:
        ...
        if fam=="pitcher_k":
            ...
            for side in ["over","under"]:
                y=_reprice_from_group_v766(grp,anchor_line,side)
                if y:
                    y["game_pk"]=game_pk or y.get("game_pk")
                    y["draftea_line_known"]=False; y["line_source"]="estimada por modelo"; y["needs_draftea_line"]=True
                    out.append(y)
        else:
            # V7.6.7: conserva la escalera positiva completa (1+/2+/3+...) para que el analizador
            # pueda comparar un escalón de riesgo adicional. La UI nunca muestra "Over 2.5 Hits";
            # lo presenta como 3+ Hits. HR continúa con su única línea natural.
            if fam=="home_run":
                ladder=sorted(overs,key=lambda z:float(z.get("line",0)))[:1]
            else:
                ladder=sorted(overs,key=lambda z:float(z.get("line",0)))
            for anchor in ladder:
                ...
                y["prop_ladder_level"]=prop_threshold_v767(y)
                out.append(y)
    return out

def market_conflict_key_v766(item):
...
def remove_market_conflicts_v766(items):
    out=[]; seen=set()
    for x in items:
        k=market_conflict_key_v766(x)
        if k in seen: continue
        seen.add(k); out.append(x)
    return out

def paper_pick_signature_v766(obj):
    """Firma exacta de una selección para no volver a recomendar lo ya guardado en Paper."""
    fam=str(obj.get("market_family") or _paper_market_family(obj) or "").strip().lower()
    try: game_pk=int(obj.get("game_pk",0) or 0)
    except Exception: game_pk=0
    subj=str(obj.get("subject") or "").strip().lower()
    side=str(obj.get("side") or "").strip().lower()
    try:
        raw_line=obj.get("line")
        line="" if raw_line in (None,"") else f"{float(raw_line):.3f}"
    except Exception:
        line=str(obj.get("line") or "")
    # Registros antiguos pueden no tener campos estructurados; el label/market es respaldo.
    label=str(obj.get("label") or obj.get("market") or "").strip().lower()
    return (game_pk,fam,subj,side,line,label)

def paper_exclusion_keys_v766(records,bet_date=None):
    keys=set()
    date_s=str(bet_date.isoformat() if hasattr(bet_date,"isoformat") else (bet_date or ""))
    def add_one(r):
        if date_s and str(r.get("date") or "") not in ("",date_s): return
        sig=paper_pick_signature_v766(r)
        keys.add(sig)
        # También una versión sin label para que un cambio cosmético de texto no reactive la apuesta.
        keys.add(sig[:-1]+("",))
    for rec in records or []:
        if str(rec.get("record_type") or "").upper()=="PARLAY":
            for leg in rec.get("legs") or []: add_one(leg)
        else:
            add_one(rec)
    return keys

def is_already_papered_v766(item,records,bet_date=None):
    keys=paper_exclusion_keys_v766(records,bet_date)
    sig=paper_pick_signature_v766(item)
    return sig in keys or (sig[:-1]+("",)) in keys

def filter_already_papered_v766(items,records,bet_date=None):
    kept=[]; removed=[]
    keys=paper_exclusion_keys_v766(records,bet_date)
    for item in items:
        sig=paper_pick_signature_v766(item)
        if sig in keys or (sig[:-1]+("",)) in keys:
            removed.append(item)
        else:
            kept.append(item)
    return kept,removed
...
def probability_guard_v767(item):
    """V7.6.7: capa de calibración preventiva.

    No inventa información ni cambia la simulación. Reduce sobreconfianza cuando la
    calidad/reliability/lineup/volatilidad no justifican alejarse demasiado de 50%.
    Conserva las probabilidades crudas para auditoría.
    """
    x=dict(item)
    fam=str(x.get("market_family","") or "")
    raw=float(x.get("prob",.5)); raw_low=float(x.get("prob_low",raw)); raw_high=float(x.get("prob_high",raw))
    q=clamp(float(x.get("data_quality",x.get("quality",65)))/100,.30,1.0)
    rel=clamp(float(x.get("market_reliability",market_reliability_v74(x)))/100,.35,.99)
    agr=clamp(float(x.get("agreement",.75)),.45,.99)
    confirmed=1.0 if x.get("both_lineups_confirmed",x.get("confirmed",False)) else .78
    vol=str(x.get("volatility","medium"))
    vol_factor={"low":1.0,"medium":.94,"high":.82}.get(vol,.92)
    fam_factor={"fg_ml":.96,"f5_total":.95,"fg_total":.93,"pitcher_k":.91,
                "hits":.90,"total_bases":.86,"hrr":.83,"home_run":.72}.get(fam,.88)
    evidence=clamp((.34*q+.28*rel+.22*agr+.16*confirmed)*vol_factor*fam_factor,.52,.97)
    def shrink(v,extra=1.0):
        return clamp(.5+(float(v)-.5)*evidence*extra,.01,.99)
    adj=shrink(raw,1.0)
    # El límite inferior es la cifra que más pesa en el ranking; se contrae un poco más.
    adj_low=shrink(raw_low,.94)
    adj_high=shrink(raw_high,1.03)
    if adj_low>adj: adj_low=min(adj,adj_low)
    if adj_high<adj: adj_high=max(adj,adj_high)
    x["prob_model_raw"]=raw; x["prob_low_raw"]=raw_low; x["prob_high_raw"]=raw_high
    x["prob"]=adj; x["prob_low"]=adj_low; x["prob_high"]=adj_high
    x["probability_guard_v767"]={"evidence":evidence,"shrink_pp":(raw-adj)*100}
    return x


def prop_threshold_v767(item):
    fam=str(item.get("market_family","") or "")
    if fam not in {"hits","total_bases","hrr","home_run"}: return None
    try: return max(1,int(math.floor(float(item.get("line",0)))+1))
    except Exception: return None


def prop_ladder_score_v767(item):
    """Score de comparación entre escalones de un mismo prop.
    No sustituye la probabilidad: permite detectar cuándo 2+ conserva señal suficiente
    como alternativa de riesgo moderadamente mayor.
    """
    fam=str(item.get("market_family","") or "")
    k=prop_threshold_v767(item)
    if fam not in {"hits","total_bases","hrr"} or not k: return -9
    p=float(item.get("prob",.5)); low=float(item.get("prob_low",p))
    vals=item.get("sample_values")
    try: mu=float(np.mean(vals)) if vals is not None and len(vals) else 0.0
    except Exception: mu=0.0
    # Pequeño premio a un escalón superior, pero sólo si la distribución todavía lo respalda.
    bonus={1:0.0,2:.075,3:.09,4:.07}.get(k,.0)
    need={1:(.58,.54),2:(.43,.36),3:(.24,.18),4:(.16,.11)}.get(k,(.99,.99))
    if p<need[0] or low<need[1]: return -9
    mu_need={"hits":{2:1.25,3:1.75},"total_bases":{2:1.55,3:2.15,4:2.70},"hrr":{2:1.45,3:2.05,4:2.65}}
    if k>=2 and mu < mu_need.get(fam,{}).get(k,0): return -9
    rel=float(item.get("market_reliability",market_reliability_v74(item)))/100
    q=float(item.get("data_quality",item.get("quality",65)))/100
    width=max(0,float(item.get("prob_high",p))-low)
    return low*.58+p*.17+rel*.10+q*.08+bonus-width*.11


def prop_ladder_opportunities_v767(items,limit=5):
    out=[]
    for z0 in items:
        if str(z0.get("market_family","") or "") not in {"hits","total_bases","hrr"}: continue
        if str(z0.get("side","") or "")!="over": continue
        k=prop_threshold_v767(z0)
        if k is None or k<2: continue
        z=dict(z0); sc=prop_ladder_score_v767(z)
        if sc<=-1: continue
        z["prop_ladder_score_v767"]=sc
        z["prop_ladder_level"]=k
        out.append(z)
    # Sólo una alternativa por jugador para mantener la salida simple.
    out=sorted(out,key=lambda x:x.get("prop_ladder_score_v767",-9),reverse=True)
    seen=set(); final=[]
    for z in out:
        key=(z.get("game_pk"),str(z.get("subject","")).lower())
        if key in seen: continue
        seen.add(key); final.append(z)
...
def rank_automatic_candidates_v5(items,max_items=5):
    ...
    for item in ranked:
        ...
        if cat_counts.get(item["category"],0)>=2:
            continue
        ckey=market_conflict_key_v766(item)
...
def express_safety_score_v721(item,use_odds=False):
    ...
    # V7.6.7: 2+/3+ no reemplaza el enfoque safety-first, pero puede competir si conserva
    # suficiente probabilidad y distribución esperada. El bonus está deliberadamente limitado.
    _pls=prop_ladder_score_v767(item)
    if _pls>-1:
        _lvl=prop_threshold_v767(item) or 1
        if _lvl>=2: score += min(.045,max(0.0,(_pls-.50)*.18+.012))
    if use_odds and item.get("reference_odds"):
...
def express_qualifies_v721(item,use_odds=False):
    ...
    if vol=="high":
        base=(low>=.70 and conf>=74 and bq>=72)
    else:
        base=(low>=.64 and conf>=68 and bq>=70)
    if not base:
        return False
    if use_odds:
...
def prebet_quality_gate_v76(item,use_odds=False):
    ...
    if data < 82: reasons.append(f"calidad de datos {data:.0f}/100")
    if width > .16: reasons.append("intervalo de incertidumbre demasiado amplio")
    if rel < 72: reasons.append(f"confiabilidad del mercado {rel:.0f}/100")
    if bq < 70: reasons.append(f"Bet Quality {bq:.0f}/100")
    ...
    if fam=="pitcher_k" and (conf < 70 or low < .64):
        reasons.append("Pitcher K requiere confianza >=70 y conservadora >=64%")
    if fam in {"fg_ml","f5_total","fg_total"} and low < .64:
        reasons.append("probabilidad conservadora de mercado de juego <64%")
    if fam in {"hits","total_bases","hrr"} and low < .65:
        reasons.append("prop de bateador requiere conservadora >=65%")
...
def rebuild_express_v762():
    ...
    if show_risky and len(chosen)<target_n:
        counts={}
        used_conflicts={market_conflict_key_v766(z) for z in chosen}
        ...
        for z in near:
            ...
            ckey=market_conflict_key_v766(z)
...
def build_paper_record_v762(item,odds,stake_mxn,selected_date,games,source="MANUAL",group_id=None):
    ...
    return {
        ...
        "hours_to_game_at_freeze":round(float(htg),2) if htg is not None else None,"model_version":"V7.6.9","date":selected_date.isoformat(),
...
def build_parlay_paper_record_v764(legs,stake_mxn,selected_date,games,parlay_index=1):
    ...
    return {
        ...
        "model_version":"V7.6.9","date":selected_date.isoformat(),"game_pk":0,
...
def diversify_express_v721(pool,target_n,max_per_game=1,automatic=True,allowed_groups=None):
    ...
    if not automatic: return remove_market_conflicts_v766(pool)[:target_n]
    ...
    while len(selected)<target_n and progress:
        ...
        for group in order:
            ...
            while idx[group] < len(arr):
                ...
                ckey=market_conflict_key_v766(x)
...
def analyze_game_express_v7(g,selected_date,allowed_groups=None):
    ...
    if "F5 Carreras" in allowed:
      # V7.6.7: catálogo F5 deliberadamente reducido a las dos líneas operables pedidas:
      # Over 4.5 y Under 5.5. El motor sigue simulando toda la distribución; solo cambia
      # la capa de apuestas que puede llegar al ranking.
      for line,side,word in [(4.5,"over","Over"),(5.5,"under","Under")]:
    ...
    # V7.6.7: Express usa únicamente F5 carreras O4.5/U5.5; F5 ML no entra al menú automático.
    ...
    if "Pitcher Ks" in allowed or "Batter props" in allowed:
        ...
        pr=normalize_draftea_prop_candidates_v766(pr,g.get("game_pk"),d)
    ...
    # V7.6.7: contrae probabilidades antes del ranking para reducir señales sobreconfiadas.
    # quality/confirmed ya están presentes en cada mercado; los valores crudos quedan auditables.
    _guarded=[]
    for _z0 in items:
        _z=dict(_z0)
        _z["market_reliability"]=market_reliability_v74(_z)
        _z=probability_guard_v767(_z)
        _guarded.append(_z)
    items=_guarded
...
# ================= APP UI =================
st.set_page_config(page_title="MLB Betting Hub V7.6.9", page_icon="⚾", layout="wide")
st.title("⚾ MLB Betting Hub — V7.6.9 Alpha")
st.caption("V7.6.9: F5 Manual muestra claramente el partido seleccionado y compara hasta 4 líneas reales de Draftea (Over/Under) en una sola corrida.")
st.info("✍️ **V7.6.9 ALPHA — F5 MANUAL x4** — Captura hasta 4 líneas reales de Draftea para el partido seleccionado. El motor compara Over y Under de cada línea (hasta 8 opciones) usando la misma simulación y filtros avanzados.")
...
props=normalize_draftea_prop_candidates_v766(props,game.get("game_pk"),selected_date.isoformat())
...
# V7.6.7: en F5 solo se presentan las dos líneas de trabajo elegidas para Draftea.
for line,direction,label_dir in [(4.5,"over","Over"),(5.5,"under","Under")]:
...
# V7.6.7: F5 ML deja de formar parte de las apuestas automáticas del análisis individual.
# Para F5, la capa apostable queda únicamente en Over 4.5 y Under 5.5.
...
with tabManualF5:
    st.subheader("✍️ F5 Manual — líneas reales de Draftea")
    st.markdown(f"### ⚾ Partido seleccionado: **{game['away_abbr']} @ {game['home_abbr']}**")
    st.caption(f"{game.get('label', game['away_abbr']+' @ '+game['home_abbr'])} · {format_game_time_cdmx(game.get('game_time_local'))} · Lineups {'✅ confirmados' if both_confirmed else '⚠️ pendientes'}")
    st.caption("Captura exactamente las líneas y momios que ves en Draftea. Puedes comparar 4 líneas a la vez; se evalúan Over y Under de cada una, hasta 8 opciones reales.")

    st.markdown("#### 📝 Captura hasta 4 líneas F5")
    defaults=[3.5,4.5,5.5,6.5]
    manual_specs=[]
    hdr=st.columns([0.55,1,1,1])
    hdr[0].markdown("**Usar**")
    hdr[1].markdown("**Línea F5**")
    hdr[2].markdown("**Momio Over**")
    hdr[3].markdown("**Momio Under**")
    for _i,_default in enumerate(defaults, start=1):
        _c0,_c1,_c2,_c3=st.columns([0.55,1,1,1])
        _use=_c0.checkbox(f"#{_i}",value=True,key=f"v769_f5_use_{game['game_pk']}_{_i}",label_visibility="visible")
        _line=float(_c1.number_input(f"Línea {_i}",min_value=0.5,max_value=15.5,value=float(_default),step=0.5,key=f"v769_f5_line_{game['game_pk']}_{_i}",label_visibility="collapsed"))
        _over=float(_c2.number_input(f"Over {_i}",min_value=1.01,max_value=100.0,value=1.90,step=0.01,format="%.2f",key=f"v769_f5_over_{game['game_pk']}_{_i}",label_visibility="collapsed"))
        _under=float(_c3.number_input(f"Under {_i}",min_value=1.01,max_value=100.0,value=1.90,step=0.01,format="%.2f",key=f"v769_f5_under_{game['game_pk']}_{_i}",label_visibility="collapsed"))
        if _use:
            manual_specs.append({"slot":_i,"line":_line,"over_odds":_over,"under_odds":_under})

    st.caption(f"Modelo base actual: **{game['away_abbr']} {away_f5:.2f} + {game['home_abbr']} {home_f5:.2f} = {f5_total:.2f} carreras F5** · Calidad {quality}/100")

    _lines=[round(float(x['line']),3) for x in manual_specs]
    if len(_lines)!=len(set(_lines)):
        st.warning("Tienes líneas F5 repetidas. Puedes analizarlas, pero normalmente conviene capturar cada línea de Draftea una sola vez.")

    if st.button("🧠 Analizar las líneas F5",key=f"v769_analyze_manual_f5_{game['game_pk']}",type="primary",use_container_width=True):
        if not manual_specs:
            st.warning("Activa al menos una línea para analizar.")
        else:
            manual_results=[]
            for _spec in manual_specs:
                _line=float(_spec["line"])
                for _side,_word,_odds in [("over","Over",float(_spec["over_odds"])),("under","Under",float(_spec["under_odds"]))]:
                    _center=sim_total_prob(f5_sim,_line,_side)
                    _scenario=scenario_total_probs(away_models,home_models,_line,_side)
                    _p,_lo,_hi,_agreement=conservative_probability(_center,_scenario,quality,both_confirmed,"medium")
                    _item={
                        "category":"F5","label":f"F5 {_word} {_line:g}",
                        ...
                        "volatility":"medium","market_family":"f5_total","side":_side,"line":_line,
                        "subject":f"{game['away_abbr']} @ {game['home_abbr']}","sample_values":f5_sim["total"],
                        "game":game["label"],"game_pk":game["game_pk"],"manual_slot":int(_spec["slot"]),
            ...
            st.session_state[f"v769_manual_f5_results_{game['game_pk']}"]=manual_results
            st.session_state[f"v769_manual_f5_specs_{game['game_pk']}"]=[dict(x) for x in manual_specs]

    manual_results=st.session_state.get(f"v769_manual_f5_results_{game['game_pk']}",[])
    _saved_specs=st.session_state.get(f"v769_manual_f5_specs_{game['game_pk']}",[])
    if manual_results:
        def _spec_signature(_arr):
            return sorted((int(x.get('slot',0)),round(float(x.get('line',0)),3),round(float(x.get('over_odds',0)),3),round(float(x.get('under_odds',0)),3)) for x in _arr)
        stale=_spec_signature(manual_specs)!=_spec_signature(_saved_specs)
        if stale:
            st.warning("Cambiaste una línea, un momio o qué filas están activas. Pulsa **Analizar las líneas F5** para actualizar el ranking.")
        else:
            ranked_manual=sorted(manual_results,key=lambda x:(x["price_metrics"]["verdict"]=="APOSTAR",x["price_metrics"]["verdict"]=="CANDIDATO",x["price_metrics"]["ev_cons"],x.get("prob_low",0)),reverse=True)
            best=ranked_manual[0]
            best_price=best["price_metrics"]

            st.markdown("### 🏆 Resultado rápido")
            if best_price["verdict"]=="APOSTAR":
                st.success(f"✅ **MEJOR F5: {best['label']} @ {best['manual_odds']:.2f}x** · Conservadora {best['prob_low']*100:.1f}% · EV {best_price['ev_cons']*100:+.1f}%")
            elif best_price["verdict"]=="CANDIDATO":
                st.warning(f"🟡 **MEJOR CANDIDATA: {best['label']} @ {best['manual_odds']:.2f}x** · Conservadora {best['prob_low']*100:.1f}% · EV {best_price['ev_cons']*100:+.1f}%")
            else:
                st.info("⚪ **PASS** — Ninguna de las líneas capturadas tiene suficiente respaldo/precio para apostar según el modelo actual.")

            st.markdown("#### 📋 Todas las opciones, ordenadas")
            for _rank,_item in enumerate(ranked_manual, start=1):
                _pm=_item["price_metrics"]
                _icon="🟢" if _pm["verdict"]=="APOSTAR" else "🟡" if _pm["verdict"]=="CANDIDATO" else "⚪"
                st.markdown(f"**{_rank}. {_icon} {_item['label']} @ {_item['manual_odds']:.2f}x** — conservadora {_item['prob_low']*100:.1f}% · ajustada {_item['prob']*100:.1f}% · EV {_pm['ev_cons']*100:+.1f}% · **{_pm['verdict']}**")

            with st.expander("🔬 Ver análisis detallado de las opciones",expanded=False):
                for _rank,_item in enumerate(ranked_manual, start=1):
                    _pm=_item["price_metrics"]
                    _icon="🟢" if _pm["verdict"]=="APOSTAR" else "🟡" if _pm["verdict"]=="CANDIDATO" else "⚪"
                    with st.container(border=True):
                        st.markdown(f"### {_rank}. {_icon} {_item['label']} @ {_item['manual_odds']:.2f}x")
                        ...
                        with st.expander("🧪 Guardar esta F5 en Paper Bets",expanded=False):
                            _safe_line=str(_item['line']).replace('.','_')
                            _stake=st.number_input("Apuesta simulada (MXN)",min_value=1.0,max_value=10000.0,value=5.0,step=1.0,key=f"v769_stake_{game['game_pk']}_{_item['side']}_{_safe_line}")
                            if st.button("🧊 Guardar F5 manual",key=f"v769_save_{game['game_pk']}_{_item['side']}_{_safe_line}",use_container_width=True):
                                _save=dict(_item)
            ...
            if item.get("market_family") in {"pitcher_k","hits","total_bases","hrr","home_run"}:
                ...
                with st.expander(f"✏️ Ajustar línea Draftea · {item.get('subject','')}",expanded=False):
                    ...
                    if _ifam in {"hits","total_bases","hrr","home_run"}:
                        ...
                        _ithr=int(st.number_input("Objetivo Draftea (1+, 2+, 3+ …)",min_value=1,max_value=10,value=_idef,step=1,key=f"v766_game_thr_{game['game_pk']}_{i}_{_ifam}"))
                        _iline=float(_ithr)-.5
                    else:
                        _iline=st.number_input("Línea Draftea",value=float(item.get("line",4.5)),step=.5,key=f"v766_game_line_{game['game_pk']}_{i}_{_ifam}")
                    if st.button("💾 Guardar línea y recalcular partido",key=f"v766_game_save_{game['game_pk']}_{i}_{_ifam}",use_container_width=True):
                        save_draftea_line_v766(item,float(_iline),selected_date.isoformat())
                        st.session_state["v653_analysis_ready"]=True
...
with tabExpress:
    ...
    allowed_groups=st.multiselect(
        ...
        help="F5 analiza únicamente Over 4.5 y Under 5.5. En Full Game puedes elegir por separado ganador (ML) y carreras Over/Under. Pitcher Ks y Batter props son opcionales."
    )
    max_per_game=int(st.number_input("Máximo de selecciones por partido",min_value=1,max_value=3,value=1,step=1,help="Permite hasta 1, 2 o 3 selecciones del mismo juego. V7.6.7 nunca ocupa dos lugares con lados contradictorios del mismo mercado."))
    show_risky=st.toggle("Mostrar también candidatos amarillos (solo observar)",value=False,help="Apagado por defecto: Express no rellena el Top con apuestas débiles. Encendido: muestra candidatos para revisar, separados de las recomendaciones APOSTAR.")
    st.caption("🎯 V7.6.7 prioriza selectividad: si solo hay 2 apuestas que pasan el filtro, muestra 2. No rellena la cantidad pedida salvo que actives los candidatos amarillos.")
    _saved_lines=draftea_line_overrides_v766()
    ...
    if _today_saved:
        with st.expander(f"📌 Líneas Draftea recordadas hoy ({len(_today_saved)})",expanded=False):
            ...
            if st.button("🧹 Borrar todas las líneas recordadas de hoy",key="v766_clear_today_lines"):
                rows={k:v for k,v in _saved_lines.items() if not str(k).startswith(_today_prefix)}
                st.session_state["v766_draftea_market_lines"]=rows; _write_draftea_lines_v766(rows); st.rerun()
    ...
    if st.button("⚡ Buscar mejores oportunidades ahora",type="primary",use_container_width=True,key="v72_express_run"):
        ...
        # V7.6.7: una selección ya congelada en Paper (incluidas piernas de parlays)
        # no vuelve a competir ni a aparecer en una nueva búsqueda de la misma fecha.
        allp,_paper_removed=filter_already_papered_v766(allp,st.session_state.get("v653_paper_bets",[]),selected_date)
        st.session_state["v766_excluded_paper_count"]=len(_paper_removed)
        ...
        near=sorted(near,key=lambda z:z.get("express_safety_score",-9),reverse=True)
        # Alternativas 2+/3+ de riesgo adicional: se muestran aparte, nunca se disfrazan de verde.
        st.session_state["v767_prop_ladder"]=prop_ladder_opportunities_v767(prepool,limit=6)
        ...
        if show_risky and len(chosen)<target_n:
            ...
            game_counts={}
            used_conflicts={market_conflict_key_v766(z) for z in existing}
        ...
        # Mejor ganador por partido: únicamente Full Game ML cuando ese mercado está habilitado.
        ...
        st.session_state["v766_show_risky"]=show_risky
    ...
    if counts:
        ...
        _px=int(st.session_state.get("v766_excluded_paper_count",0) or 0)
        if _px:
            st.caption(f"🧪 {_px} selección(es) ya guardadas en Paper fueron excluidas automáticamente de esta búsqueda.")
    ...
    if display_express:
        # Salida principal estilo menú: rápida, operable y sin ruido. Los diagnósticos quedan abajo.
        st.markdown("## 📋 Menú rápido para apostar")
        _quick_green=express
        _quick_yellow=fallback if st.session_state.get("v766_show_risky",st.session_state.get("v724_show_risky",False)) else []
        _group_titles={"Full Game ML":"Juego Completo · ML","Full Game Carreras":"Juego Completo · Carreras","F5 Carreras":"F5 · Primeras 5 entradas","Pitcher Ks":"Pitchers","Batter props":"Hits y Bases"}
        if _quick_green:
            _qgroups={}
            for _q in _quick_green:
                _qgroups.setdefault(market_group_v722(_q),[]).append(_q)
            for _g in ["Full Game ML","Full Game Carreras","F5 Carreras","Pitcher Ks","Batter props"]:
                if _g not in _qgroups: continue
                st.markdown(f"**{_group_titles.get(_g,_g)}**")
                for _q in _qgroups[_g]:
                    st.markdown(f"🔹 **{_q.get('label','')}** — {_q.get('game','')} · conservadora {_q.get('prob_low',0)*100:.1f}%")
            st.caption("🟢 Solo aparecen aquí selecciones que pasaron el filtro APOSTAR V7.6.7. Si pediste más y no aparecen, el sistema prefirió no rellenar con señales débiles.")
        else:
            st.warning("Hoy no hay selecciones que pasen el filtro estricto APOSTAR con los mercados elegidos.")

        _ladder=st.session_state.get("v767_prop_ladder",[])
        if _ladder:
            with st.expander("⚡ Escalón + riesgo: 2+ Hits / 2+ Bases / 2+ HRR",expanded=False):
                st.caption("Estas NO son equivalentes a un verde conservador. El motor sólo las muestra cuando el segundo escalón conserva señal suficiente después del ajuste de probabilidad.")
                for _z in _ladder:
                    _raw=_z.get("prob_model_raw")
                    _rawtxt=f" · modelo crudo {_raw*100:.1f}%" if isinstance(_raw,(int,float)) else ""
                    st.markdown(f"🔸 **{_z.get('label','')}** — {_z.get('game','')} · ajustada {_z.get('prob',0)*100:.1f}% · conservadora {_z.get('prob_low',0)*100:.1f}%{_rawtxt}")
        if fallback:
            st.caption(f"🟡 Hay {len(fallback)} candidato(s) amarillo(s) opcionales; no forman parte del menú principal de apuesta.")

        with st.expander("🔬 Ver análisis detallado / diagnóstico",expanded=False):
            for _q in express:
                _raw=_q.get("prob_model_raw")
                _rawtxt=f" · Cruda {_raw*100:.1f}%" if isinstance(_raw,(int,float)) else ""
                st.write(f"• {_q.get('game','')} · {_q.get('label','')} · Ajustada {_q.get('prob',0)*100:.1f}% · Conservadora {_q.get('prob_low',0)*100:.1f}%{_rawtxt} · Conf. {_q.get('confidence_score',0)}/100 · Bet Quality {_q.get('bet_quality_score',0)}/100")
        ...
        for i,x in enumerate(display_express,1):
            ...
            with st.container(border=True):
                ...
                if _gate.get("pass"):
                    st.caption(f"🛡️ Gate V7.6.7: APOSTAR · Statcast lineup {float(x.get('statcast_coverage',0))*100:.0f}%")
                else:
                    st.caption("🛡️ Gate V7.6.7: REVISAR · " + " · ".join(_gate.get("reasons",[])[:4]))
                ...
                if x.get("sample_values") is not None:
                    with st.expander("✏️ Draftea tiene otra línea / editar esta apuesta"):
                        ...
                        if rx:
                            ...
                            if x.get("draftea_line_known") and st.button("🗑️ Olvidar línea Draftea guardada",key=f"v766_forget_{x['game_pk']}_{i}",use_container_width=True):
                                delete_draftea_line_v766(x,selected_date.isoformat())
                            ...
                            if st.button("🔄 Aplicar cambio y actualizar TODO Express",key=f"v721_apply_{x['game_pk']}_{i}",use_container_width=True):
                                ...
                                rx["label"]=market_display_label_v766(rx)
                                rx["draftea_line_known"]=True; rx["line_source"]="Draftea manual"; rx["needs_draftea_line"]=False
                                save_draftea_line_v766(x,float(line),selected_date.isoformat())
                                ...
                                if fam=="pitcher_k":
                                    for sside in ["over","under"]:
                                        ...
                                        if rz:
                                            rz["game_pk"]=x.get("game_pk"); rz["game"]=x.get("game"); rz["label"]=market_display_label_v766(rz)
                ...
                with st.expander("🧪 Guardar esta selección en Paper Bets"):
                    ...
                    if st.button("🧊 Guardar en Paper Bets",key=f"v762_exp_paper_save_{x.get('game_pk')}_{i}",use_container_width=True):
                        ...
                        if _ok:
                            # Desaparece de Express inmediatamente y seguirá excluida en futuras búsquedas.
                            _recs=st.session_state.get("v653_paper_bets",[])
                            for _k in ["v7_express_results","v724_express_fallback","v723_express_near","v762_express_prepool"]:
                                _arr=st.session_state.get(_k,[])
                                st.session_state[_k]=[z for z in _arr if not is_already_papered_v766(z,_recs,selected_date)]
                            st.success("Paper Bet guardada. Ya no volverá a salir como recomendación hoy.")
                            st.rerun()
...
with tabParlay:
    ...
    st.caption("Este módulo NO depende de Express. Puedes pedir 1 a 5 parlays; cada búsqueda revisa toda la jornada, excluye juegos iniciados y también selecciones ya guardadas en Paper para no duplicar apuestas.")
    ...
    if parlay_profile=="Menor riesgo":
        st.info("🛡️ Menor riesgo: prioriza probabilidad conservadora, confianza, Bet Quality y Gate V7.6.7. Si no alcanza la calidad, puede entregar menos parlays o menos piernas.")
    ...
    if st.button("🎟️ Buscar parlays en toda la jornada",type="primary",use_container_width=True,key="v763_build_independent_parlays"):
        ...
        candidates,_parlay_paper_removed=filter_already_papered_v766(candidates,st.session_state.get("v653_paper_bets",[]),selected_date)
        st.session_state["v766_parlay_excluded_paper_count"]=len(_parlay_paper_removed)
                    ...
                    for spec in edit_specs:
                        ...
                        if base.get("sample_values") is not None and spec.get("line") is not None:
                            ...
                            if base.get("market_family") in {"pitcher_k","hits","total_bases","hrr","home_run"}:
                                save_draftea_line_v766(base,float(spec["line"]),selected_date.isoformat())
                                y["draftea_line_known"]=True; y["line_source"]="Draftea manual"; y["needs_draftea_line"]=False
                                y["label"]=market_display_label_v766(y)
                        else:
                            y=dict(base)
...
with tab2:
    ...
    if not source_candidates:
        ...
        # Hotfix V7.6.7: algunos candidatos nuevos pueden traer `market`/`subject`
